...
if [ $platform == 'ont' ]; then
	...
	configfile=$thisdir/configfiles/pbcr_sens.spec
	...
	if [ $strain == 's288c' ] && [ -z ${cov-x} ]; then
        ...
        if  [ ! -z ${cov-x} ]; then
			...
			configfile=$thisdir/configfiles/pbcr.spec
                fi	
	else	
		if [ $strain == 's288c' ]; then
                	echo; echo "For pacbio s288c you should choose if running the sample with all data (allX) or the subsample with '31X' coverage"; echo
...
if [ ! -f $reads ]; then
	...
	$myexe -length 500 -partitions 200 -l $strain -s $(basename $configfile) -fastq $reads  genomeSize=12160000
